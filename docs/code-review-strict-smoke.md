...
In this environment, `@chrome-mcp` implements child-review isolation by opening
and separately addressing distinct ChatGPT conversations in separate browser tabs.
A browser tab is not sufficient by itself: the tabs must point at different ChatGPT
conversation URLs/IDs and must remain independently addressable for the whole review.
...
- `PASS` — two separately addressable browser tabs contained distinct ChatGPT
  conversations, both independently accessed GitHub and resolved the same pinned head,
  and neither showed the sibling canary.
...
- `NOT EXERCISED` — the current environment cannot create/address two distinct
  ChatGPT conversations in separate tabs, or the synthetic canary has not been run.

Separate `@chrome-mcp` tabs are therefore a valid child-review mechanism when they
contain distinct ChatGPT conversations and satisfy the canary. Two tabs showing the
same conversation, or two sequential prompts in one conversation, are not equivalent.
