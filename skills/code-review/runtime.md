...
Only treat a child as failed when collection reports a genuinely terminal failure (for example `FAILED_TERMINAL` / run `FAILED`), a defined bounded polling/timeout policy expires, or the user explicitly cancels. Do not inspect, close, cancel, or aggregate any child early while it is recoverable, even if another child already produced a verified result. Aggregate only after the required collection barrier is satisfied.
